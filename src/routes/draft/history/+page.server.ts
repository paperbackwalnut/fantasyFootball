...
export const load: PageServerLoad = async () => {
	...
	return { sessions: rows.map((row) => {
		...
		const advice = [...latestByPick.values()].map((snapshot) => ({ currentPick: snapshot.current_pick, recommendations: JSON.parse(snapshot.recommendations_json), capturedAt: snapshot.created_at }));
