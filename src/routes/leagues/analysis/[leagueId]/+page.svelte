<script lang="ts">
	...
	const lineupPlayers = $derived(['Hold', 'Unavailable'].includes(data.startSit.confidence) ? (data.user?.currentStarters ?? []) : (data.user?.starters ?? []));
	const nextKickoff = $derived(data.kickoffSchedule?.nextGame?.kickoffAt ? new Date(data.kickoffSchedule.nextGame.kickoffAt) : null);
	function kickoffLabel(player: any) { return player.kickoffAt ? new Date(player.kickoffAt).toLocaleString([], { weekday: 'short', hour: 'numeric', minute: '2-digit' }) : 'Game time unverified'; }
	function injuryLabel(player: any) { return player.injuryStatus && !['NA', 'ACTIVE'].includes(String(player.injuryStatus).toUpperCase()) ? String(player.injuryStatus) : 'No injury designation'; }
...
<main class="dark-shell min-h-screen px-4 py-8 sm:px-8">
	<div class="mx-auto max-w-7xl">
		...
		<section class="border-b border-slate-300 py-8">
			...
			{#if data.startSit.start.length || data.startSit.sit.length}
				<div class="grid gap-4 md:grid-cols-2">
					<div class="rounded-xl border-2 border-emerald-500 bg-emerald-950/30 p-5"><h3 class="text-sm font-black uppercase tracking-widest text-emerald-300">Start these guys</h3>{#each data.startSit.start as player}<div class="mt-4 border-t border-emerald-800 pt-3"><p class="text-xl font-black">{player.name}</p><p class="mt-1 text-sm text-slate-300">{player.position} · {player.weeklyProjected} projected pts · {player.opponent ?? 'Opponent unverified'}</p><p class="mt-1 text-xs text-slate-400">{kickoffLabel(player)} · {injuryLabel(player)}</p></div>{/each}</div>
					<div class="rounded-xl border-2 border-rose-500 bg-rose-950/30 p-5"><h3 class="text-sm font-black uppercase tracking-widest text-rose-300">Sit these guys</h3>{#each data.startSit.sit as player}<div class="mt-4 border-t border-rose-800 pt-3"><p class="text-xl font-black">{player.name}</p><p class="mt-1 text-sm text-slate-300">{player.position} · {player.weeklyProjected} projected pts · {player.opponent ?? 'Opponent unverified'}</p><p class="mt-1 text-xs text-slate-400">{kickoffLabel(player)} · {injuryLabel(player)}</p></div>{/each}</div>
				</div><p class="mt-3 text-sm font-semibold text-slate-400">{data.startSit.confidence} confidence · {data.startSit.edge} projected points across all changes. Make these moves on {isSleeper ? 'Sleeper' : 'ESPN'}.</p>
			{:else if data.startSit.confidence === 'Hold' || data.startSit.confidence === 'Unavailable'}
				<div class="rounded-xl border-2 border-amber-500 bg-amber-950/30 p-5"><h3 class="text-xl font-black text-amber-300">Keep your current lineup for now</h3><p class="mt-2 text-sm text-slate-300">No safe start/sit change is recommended.</p>{#each data.startSit.warnings as warning}<p class="mt-2 text-sm text-amber-200">• {warning}</p>{/each}{#if !data.startSit.warnings.length && data.startSit.edge != null}<p class="mt-2 text-sm text-slate-300">The projected gain is only {data.startSit.edge} points, or the weekly ranking disagrees.</p>{/if}</div>
			{:else}<div class="rounded-xl border-2 border-emerald-700 bg-emerald-950/30 p-5"><h3 class="text-xl font-black text-emerald-300">No changes recommended</h3><p class="mt-2 text-sm text-slate-300">Your submitted lineup matches the current recommendation.</p></div>{/if}
	...
	{#if reviewingLineup}
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4" role="presentation">
			<section class="detail-panel max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-slate-700 p-6" aria-label="Review lineup changes">
				...
				<div class="mt-6 grid gap-4 sm:grid-cols-2"><div><h3 class="font-black text-emerald-400">Start these guys</h3>{#each data.startSit.start as player}<p class="mt-3 font-bold">{player.name} · {player.position}</p><p class="text-xs text-slate-400">{kickoffLabel(player)} · {injuryLabel(player)}</p>{/each}</div><div><h3 class="font-black text-rose-400">Sit these guys</h3>{#each data.startSit.sit as player}<p class="mt-3 font-bold">{player.name} · {player.position}</p><p class="text-xs text-slate-400">{kickoffLabel(player)} · {injuryLabel(player)}</p>{/each}</div></div>
