const defaultRoster = { QB: 1, RB: 2, WR: 2, TE: 1, FLEX: 1, DST: 1, K: 1, BENCH: 7 };
...
export function deriveLeagueContext(state, importedLeague) {
	...
	return {
		...
		scoring: scoringLabel(espnSettings?.scoringSettings), rosterSlots: espnSettings?.rosterSettings?.lineupSlotCounts ?? defaultRoster,
...
function scoringLabel(settings) {
	if (!settings) return { format: 'UNKNOWN', receptionPoints: null };
	const reception = settings.scoringItems?.find((/** @type {any} */ item) => Number(item.statId) === 53)?.points;
	return { format: reception === 1 ? 'PPR' : reception === 0.5 ? 'HALF_PPR' : reception === 0 ? 'STANDARD' : 'CUSTOM', receptionPoints: reception ?? null };
}
